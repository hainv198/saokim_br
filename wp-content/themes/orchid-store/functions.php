...
add_action('demo_action','demo_hook');


// filter 

function func_demo_filter($str) {
	$str = 'Filter to edit';
	return $str;
}
add_filter( 'demo_filter', 'func_demo_filter');


// the_contetn

function keyword($content) {
	$find = 'Bạn có thể là một nghệ sĩ';
	$replacement = '<b> Hihihahaha </b>';
    $content = str_replace($find, $replacement, $content);
	return $content;
}

add_filter( 'the_content', 'keyword' );

function path_url($content) {
	$url_path = parse_url( $_SERVER['REQUEST_URI'], PHP_URL_PATH );
	if(strpos($url_path,'/saokim/gioi-thieu/') !== false) {
		$alert_message = '<script>console.log("Xin chào! Bạn đang truy cập vào trang giới thiệu của Saokim");</script>';
...
add_filter( 'the_content', 'path_url' );
